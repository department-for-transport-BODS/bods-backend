...
class DynamoDBLoader:
    ...
    def create_table_if_not_exists(self):
        ...
        try:
            ...
            self.log.info("DynamoDB table already exists")
        except ClientError as e:
            error_response: _ClientErrorResponseTypeDef = e.response
            if (
                ...
                self.log.info("Creating new DynamoDB table")

                try:
                    self.table = self.dynamodb.create_table(
                        TableName=self.table_name,
                        KeySchema=[{"AttributeName": "ATCOCode", "KeyType": "HASH"}],
                        AttributeDefinitions=[
                            {"AttributeName": "ATCOCode", "AttributeType": "S"},
                    ...
                    self.log.info("Successfully created DynamoDB table")
                except ClientError as create_error:
                    self.log.error(
                        "Failed to create DynamoDB table",
                        error_code=create_error.response.get("Error", {}).get("Code"),
                        error_message=create_error.response.get("Error", {}).get(
                            "Message"
                        ),
                    )
                    raise
