...
class OtcServiceRepository:
    ...
    def _add_traveline_region_weca(self, session, query):
        ...
        traveline_region_weca_subquery = (
            session.query(
                func.string_agg(NaptanAdminArea.traveline_region_id, text("'|'")).label("traveline_region_weca"),
    ...
    def _add_traveline_region_otc(self, session, query):
        ...
        traveline_region_otc_subquery = (
            session.query(
                func.string_agg(NaptanAdminArea.traveline_region_id, text("'|'")).label("traveline_region_otc"),
                OtcService.id,
            )
            .join(
            ...
            .join(OtcLocalAuthority, OtcLocalAuthority.id == OtcLocalAuthorityRegistrationNumbers.localauthority_id)
    ...
    def get_service_with_traveline_region(self, registration_number: str) -> ServiceWithRegion | None:
        try:
            with self._db.session as session:
                ...
                base_query = session.query(OtcService).filter(
                    OtcService.registration_number == registration_number.replace(":", "/")
                )

                # Add traveline_region_weca and traveline_region_otc columns
                query_with_traveline_region_weca, weca_column_label = self._add_traveline_region_weca(
                    session, base_query
                )
                query_with_traveline_region_otc, otc_column_label = self._add_traveline_region_otc(
                    session, query_with_traveline_region_weca
                )

                # Add traveline_region details based on api type
                query_with_traveline_region = query_with_traveline_region_otc.add_columns(
                    case(
                        (OtcService.api_type.in_(["WECA", "EP"]), weca_column_label),
