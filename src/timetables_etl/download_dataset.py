...
def download_data_from_remote_url(revision):
    try:
...
def get_remote_file_name(revision, response, is_time_zone):
...
def download_and_upload_dataset(input_data, TIME_ZONE):
    """
    ...
    file_name = get_remote_file_name(revision, response, TIME_ZONE)
...
def update_dataset_revision(revision_id, file_name, db):
    """
